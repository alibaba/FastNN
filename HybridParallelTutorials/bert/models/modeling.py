...
def gelu(input_tensor):
  ...
  """
  cdf = 0.5 * (1.0 + tf.erf(tf.cast(input_tensor, tf.float32) / tf.sqrt(2.0)))
  return input_tensor * tf.cast(cdf, input_tensor.dtype)
...
def get_assigment_map_from_checkpoint(tvars, init_checkpoint, scope_map=None):
    ...
    #logging.info('assignment_map')
    #for key, value in assignment_map.iteritems():
...
def embedding_lookup(input_ids,
  ...
  embedding_table = tf.get_variable(
      ...
      partitioner=tf.min_max_variable_partitioner(max_partitions=12, min_slice_size=4<<20))
...
def attention_layer(from_tensor,
  ...
  ########
  # NOTE(zycao): Since TF with CUDA 9.0 cannot do high performance FP16 Batch
  # Matmul and would cause 10x performance drop here, we use FP32 at this time.
  ########
  # Take the dot product between "query" and "key" to get the raw
  # attention scores.
  # `attention_scores` = [B, N, F, T]
  attention_scores = tf.matmul(tf.cast(query_layer, tf.float32),
                               tf.cast(key_layer, tf.float32), transpose_b=True)
...
def transformer_model(input_tensor,
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.variable_scope("layer_%d" % (layer_idx + start_layer_idx)):
