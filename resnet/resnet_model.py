...
# =============================================================================
"""Resnet model def. The code is modified from
   https://github.com/tensorflow/models/blob/v1.12.0/official/resnet/resnet_model.py"""
...
def batch_norm_relu(inputs, is_training, data_format):
  ...
  inputs = tf.layers.batch_normalization(
      ...
      #trainable=False) # trainable param, sky adding
...
def imagenet_resnet_v2_generator(block_fn, layers,
  ...
  def model(inputs, is_training):
    ...
    #### delete fc for data parallel
    #inputs = tf.layers.dense(inputs=inputs, units=num_classes)
    #inputs = tf.identity(inputs, 'stanford_196')
    return inputs

  return model


def imagenet_resnet_v2(resnet_size, data_format = None):
  """Returns the ResNet model for a given size and number of output classes."""
  model_params = {
      10: {'block': building_block, 'layers': [1, 1, 1, 1]}, # added by yh
